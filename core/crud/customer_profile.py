...
#         profiles = (
#             await models.CustomerProfile.objects.offset(offset).limit(limit).all()
#         )
#         if not profiles:
...
#             hiv_status: str
#     ) -> Union[models.CustomerProfile, Response]:
#         customer_hiv_status_list = (
#             await models.CustomerProfile.objects.offset(offset)
#             .limit(limit)
#             .filter(hiv_status=hiv_status)
#             .all()
#         )
#         if not customer_hiv_status_list:
#             return Response(status_code=status.HTTP_404_NOT_FOUND)
#         return customer_hiv_status_list
#
#     async def get_by_id(
#             self,
#             _id: int
