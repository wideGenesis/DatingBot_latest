...
class CreateAdvGoalsDialog(ComponentDialog):
    def __init__(
        ...
        super(CreateAdvGoalsDialog, self).__init__(dialog_id or CreateAdvGoalsDialog.__name__)
        ...
        self.add_dialog(TextPrompt(TextPrompt.__name__, CreateAdvGoalsDialog.answer_prompt_validator))
        self.add_dialog(NumberPrompt(NumberPrompt.__name__, CreateAdvGoalsDialog.age_prompt_validator))
    ...
    async def looking_for_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        ...
        _result_from_previous_step = str(result_from_previous_step[0]).strip() + str(
            result_from_previous_step[1]).strip()
    ...
    async def goals_type_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
