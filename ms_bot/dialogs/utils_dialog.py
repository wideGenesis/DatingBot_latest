...
async def _count_broadcast_geo() -> str:
    ...
    try:
        ...
        customer = "db error" + str(e)
    return "Общее количество пользователей получивших запрос геолокации: " + str(
        customer
    )
...
async def _count_replies_received() -> str:
    ...
    try:
        ...
        customer = "db error" + str(e)
    return "Общее количество пользователей ответивших на запрос геолокации: " + str(
        customer
    )
...
async def _search_banned():
    ...
    try:
        ...
        logger.exception(
            "DB error while getting filter(operator_displayName=bot was banned)"
        )
        customers = None
...
async def _self_delete(conversation_state, step_context: WaterfallStepContext) -> bool:
    ...
    try:
        ...
        logger.warning(
            "SELECT AFTER Customer.objects.get %s object not found", member_id
        )
        select_result = True
...
def ping_message():
    ...
    message = (
        f"Час відповіді вказано у мс (1 сек = 1000 мс) \n  \n"
