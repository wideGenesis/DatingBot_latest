...
class AdvListManagementDialog(ComponentDialog):
    def __init__(
            self,
        ...
        super(AdvListManagementDialog, self).__init__(
            dialog_id or AdvListManagementDialog.__name__
        )
        ...
        self.add_dialog(
            TextPrompt(
                TextPrompt.__name__, AdvListManagementDialog.answer_prompt_validator
            )
        )

        self.add_dialog(
            WaterfallDialog(
                "MainAdvListManagementDialog",
                [
                    self.show_file_step,
        ...
        self.initial_dialog_id = "MainAdvListManagementDialog"
        self.file = None

    async def show_file_step(
            self, step_context: WaterfallStepContext
    ) -> DialogTurnResult:
        logger.debug("show_file_step %s", AdvListManagementDialog.__name__)
    ...
    async def parse_choice_step(
        ...
        logger.debug("parse_choice_step %s", AdvListManagementDialog.__name__)
        try:
