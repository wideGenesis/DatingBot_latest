...
class MyAdvDialog(ComponentDialog):
    def __init__(
        ...
        self.add_dialog(TextPrompt(TextPrompt.__name__, MyAdvDialog.answer_prompt_validator))
    ...
    async def passcode_required_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
    ...
    async def show_adv_0_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
    ...
    async def show_adv_1_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
    ...
    async def passcode_update(passcode):
        salt = uuid.uuid4().hex
        return hashlib.sha256(salt.encode() + passcode.encode()).hexdigest() + ':' + salt
