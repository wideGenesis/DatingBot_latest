...
class MainDialog(ComponentDialog):
    def __init__(
        ...
        self.add_dialog(
            UtilsDialog(conversation_state, user_state, UtilsDialog.__name__)
        )
        self.add_dialog(
            WaterfallDialog(
                "MainDialog",
    ...
    async def routing_step(
        ...
        if not step_context.result:
            logger.info(
                "Not authorised! Routing %s to TelegramRegistrationDialog", member_id
            )
